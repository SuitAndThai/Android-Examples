...
public class SnakeView extends TileView {
	 ...
	 */
	private static final int TILE_SNAKE_BODY = 1;
	private static final int TILE_SNAKE_HEAD_AND_APPLES = 2;
	...
	public void initSnakeView() {
		...
		SharedPreferences prefActivityPrefs = PreferenceManager
				.getDefaultSharedPreferences(this.getContext());
		String snakeBodySelection = prefActivityPrefs
				.getString(r.getString(R.string.key_snake_body),
						r.getString(R.string.red));
		String snakeHeadAndApplesSelection = prefActivityPrefs.getString(
				r.getString(R.string.key_snake_head_and_apples),
				r.getString(R.string.yellow));
		String wallSelection = prefActivityPrefs.getString(
				r.getString(R.string.key_wall), r.getString(R.string.green));

		resetTiles(4);

		loadTile(TILE_SNAKE_BODY, getDrawableColor(r, snakeBodySelection));
		loadTile(TILE_SNAKE_HEAD_AND_APPLES,
				getDrawableColor(r, snakeHeadAndApplesSelection));
		loadTile(TILE_WALL, getDrawableColor(r, wallSelection));

	}

	private Drawable getDrawableColor(Resources r, String pieceSelection) {
		if (pieceSelection.equals(r.getString(R.string.red))) {
			return r.getDrawable(R.drawable.redstar);
		} else if (pieceSelection.equals(r.getString(R.string.yellow))) {
			return r.getDrawable(R.drawable.yellowstar);
		} else {
			return r.getDrawable(R.drawable.greenstar);
		}
	}
	...
	private void initNewGame() {
		...
		SharedPreferences prefActivityPrefs = PreferenceManager
				.getDefaultSharedPreferences(this.getContext());
		String speedSelection = prefActivityPrefs.getString(
				r.getString(R.string.key_starting_speed), 600 + "");

		mMoveDelay = Integer.parseInt(speedSelection);
		mScore = 0;
	}

	public void setModeDelay(int s) {
		mMoveDelay = s;
	}
	...
	private void updateWalls() {
		for (int x = 0; x < mXTileCount; x++) {
			...
			setTile(TILE_WALL, x, mYTileCount - 1);
		}
		for (int y = 1; y < mYTileCount - 1; y++) {
			setTile(TILE_WALL, 0, y);
			setTile(TILE_WALL, mXTileCount - 1, y);
		}
	}
	...
	private void updateApples() {
		for (Coordinate c : mAppleList) {
			setTile(TILE_SNAKE_HEAD_AND_APPLES, c.x, c.y);
		}
	}
	...
	private void updateSnake() {
		...
		for (Coordinate c : mSnakeTrail) {
			if (index == 0) {
				...
				setTile(TILE_SNAKE_BODY, c.x, c.y);
			}
			index++;
		}
